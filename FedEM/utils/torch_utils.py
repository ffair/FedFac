    ...
    for key in target_state_dict:
        if target_state_dict[key].data.dtype == torch.float32:
            ...
            for learner_id, learner in enumerate(learners):
                state_dict = learner.model_state

                if average_params:
                    # target_state_dict[key].data += weights[learner_id] * learner_statedict[key].data.clone()
                    # print('target_state_dict[key].data {}'.format(target_state_dict[key].data))
                    # print('weights[learner_id] {}'.format(weights[learner_id]))
                    # print('state_dict[key].data {}'.format(state_dict[key].data))
                    target_state_dict[key].data += weights[learner_id] * state_dict[key].data.clone()

                if average_gradients:
                    # if learner_statedict[key].grad is not None:
